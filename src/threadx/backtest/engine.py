...
class BacktestController:
    """
    Contrôleur pour gérer l'interruption des backtests.
    
    Permet de mettre en pause, reprendre ou arrêter un backtest en cours
    ...
    def stop(self):
        """Arrête le backtest immédiatement."""
    ...
    def resume(self):
        """Reprend le backtest après une pause."""
    ...
    def check_interruption(self, context: str = ""):
        """
        Vérifie s'il faut interrompre l'exécution.
        
        Args:
            context: Contexte pour les logs (optionnel)
            
        Raises:
            KeyboardInterrupt: Si stop demandé
        """
        if self._stop_event.is_set():
            if context:
                logger.info(f"🛑 Backtest arrêté: {context}")
            raise KeyboardInterrupt("Backtest stopped by user")
            
        # Attendre si en pause
        if self._pause_event.is_set():
            if context:
                logger.info(f"⏸️ Backtest en pause: {context}")
            while self._pause_event.is_set() and not self._stop_event.is_set():
                time.sleep(0.1)
            # Vérifier à nouveau si stop demandé pendant la pause
            if self._stop_event.is_set():
                if context:
                    logger.info(f"🛑 Backtest arrêté pendant pause: {context}")
                raise KeyboardInterrupt("Backtest stopped during pause")
            if context:
                logger.info(f"▶️ Backtest repris: {context}")
                
    @property
    def is_stopped(self) -> bool:
        """True si le backtest a été arrêté."""
        return self._cancelled
        
    @property
    def is_paused(self) -> bool:
        """True si le backtest est en pause."""
        return self._paused
...
class BacktestEngine:
    ...
    def __init__(self, 
                 ...
                 controller: Optional[BacktestController] = None):
        """
        ...
        Args:
            ...
            controller: Contrôleur pour gérer pause/stop (si None, crée automatiquement)
        """
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            self.controller.check_interruption("initialisation")
            ...
            # Point de contrôle après validation
            self.controller.check_interruption("validation")
            
            # 2. Génération signaux de trading
            signals = self._generate_trading_signals(df_1m, indicators, params)
            
            # Point de contrôle après génération signaux
            self.controller.check_interruption("génération signaux")
            
            # 3. Simulation trades et gestion positions
            trades_df = self._simulate_trades(df_1m, signals, params)
            
            # Point de contrôle après simulation trades
            self.controller.check_interruption("simulation trades")
            ...
            self.logger.warning(f"⏹️ Backtest {symbol} interrompu par l'utilisateur")
            ...
            # Résultat vide en cas d'interruption
            empty_equity = pd.Series([10000.0], index=[df_1m.index[0]])
            empty_returns = pd.Series([0.0], index=[df_1m.index[0]])
            empty_trades = pd.DataFrame(columns=['entry_ts', 'exit_ts', 'pnl', 'size', 'price_entry', 'price_exit'])
    ...
    def _simulate_trades(
        ...
        # Simulation trade par trade avec contrôles d'interruption
        total_rows = len(timestamps)
        for i, (timestamp, close_price, signal) in enumerate(zip(timestamps, closes, signal_values)):
            # Point de contrôle périodique (tous les 1000 ticks)
            if i % 1000 == 0:
                self.controller.check_interruption(f"simulation tick {i}/{total_rows}")
