...
class ThreadXApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        style.configure(
            "TLabel", background=THEME["background"], foreground=THEME["text"]
        )
        style.configure(
            "TLabelframe", background=THEME["panel"], foreground=THEME["text"]
        )
        style.configure(
            "TLabelframe.Label", background=THEME["panel"], foreground=THEME["text"]
        )
        ...
        try:
            ...
            self._logger.info(
                "IndicatorBank prêt"
                if self.bank
                else "IndicatorBank indisponible (fallback)."
            )
        ...
        try:
            ...
            self._logger.info(
                "BacktestEngine prêt"
                if self.engine
                else "BacktestEngine indisponible (fallback)."
            )
        except Exception as e:
        ...
        try:
            self.performance = (
                PerformanceCalculator() if PerformanceCalculator else None
            )
        except Exception as e:
    ...
    def _build_menu(self):
        ...
        mf.add_command(
            label="Ouvrir données...",
            command=self._open_data_dialog,
            accelerator="Ctrl+O",
        )
        ...
        m.add_cascade(label="Aide", menu=mh)
        mh.add_command(
            label="À propos",
    ...
    def _build_toolbar(self):
        ...
        ttk.Button(bar, text="▶ Run Backtest (F5)", command=self.trigger_backtest).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(bar, text="💾 Export", command=self._export_dialog).pack(
            side=tk.LEFT, padx=4
        )
        self._gpu_lbl = ttk.Label(bar, text="GPU: ?")
        self._gpu_lbl.pack(side=tk.RIGHT, padx=4)
    ...
    def _tab_home(self):
        ...
        self._home_info = ttk.Label(
            tab,
            text="Chargement BTC auto si disponible (data/crypto_data_*)",
            anchor="w",
        )
    ...
    def _tab_data(self):
        ...
        frm = ttk.LabelFrame(tab, text="Charger un fichier (parquet/csv/json)")
        frm.pack(fill=tk.X, padx=10, pady=10)
        ttk.Button(
            frm, text="Ouvrir un fichier...", command=self._open_data_dialog
        ).pack(side=tk.LEFT, padx=8, pady=8)

        info = ttk.LabelFrame(tab, text="Infos dataset")
        info.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        self._data_text = ScrolledText(
            info,
            height=12,
            bg=THEME["panel"],
            fg=THEME["text"],
        ...
        self._data_text.pack(fill=tk.BOTH, expand=True, padx=8, pady=8)
        self._data_text.config(state=tk.DISABLED)

    # === Onglet 3 : Indicators ===
    def _tab_indicators(self):
        tab = ttk.Frame(self.nb)
        self.nb.add(tab, text="🔧 Indicators")
        frm = ttk.LabelFrame(tab, text="Bollinger")
        frm.pack(fill=tk.X, padx=10, pady=10)
        ttk.Label(frm, text="period").pack(side=tk.LEFT, padx=4)
        self.bb_period = tk.IntVar(value=20)
        ttk.Spinbox(frm, from_=5, to=200, width=6, textvariable=self.bb_period).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Label(frm, text="std").pack(side=tk.LEFT, padx=4)
        self.bb_std = tk.DoubleVar(value=2.0)
        ttk.Spinbox(
            frm, from_=0.5, to=4.0, increment=0.1, width=6, textvariable=self.bb_std
        ).pack(side=tk.LEFT, padx=4)

        ttk.Button(tab, text="Regenerate", command=self._regen_indicators).pack(
            padx=10, pady=10
        )
    ...
    def _tab_optimization(self):
        tab = ttk.Frame(self.nb)
        self.nb.add(tab, text="🎯 Optimization")
        ttk.Label(
            tab,
            text="Paramétrage optimisation (placeholder)\nUtilise le même pipeline que Backtest",
        ).pack(padx=12, pady=12)

    # === Onglet 5 : Sweep (factory externe si dispo) ===
    def _tab_sweep(self):
        tab = ttk.Frame(self.nb)
        self.nb.add(tab, text="🎯 Sweep")
        ...
        ttk.Label(
            tab,
            text="Module Sweep indisponible.\nCréez threadx/ui/sweep.py avec create_sweep_page(...).",
        ).pack(padx=12, pady=12)

    # === Onglet 6 : Backtest ===
    def _tab_backtest(self):
        tab = ttk.Frame(self.nb)
        self.nb.add(tab, text="🚀 Backtest")
        top = ttk.LabelFrame(tab, text="Paramètres rapides")
        top.pack(fill=tk.X, padx=10, pady=10)

        ttk.Label(top, text="Symbol").pack(side=tk.LEFT, padx=4)
        self.symbol_var = tk.StringVar(value="BTCUSDT")
        ttk.Entry(top, width=12, textvariable=self.symbol_var).pack(
            side=tk.LEFT, padx=4
        )
        ...
        act = ttk.Frame(tab)
        act.pack(fill=tk.X, padx=10, pady=10)
        self.bt_btn = ttk.Button(
            act, text="▶ Run Backtest", command=self.trigger_backtest
        )
        self.bt_btn.pack(side=tk.LEFT, padx=4)
        self.bt_prog = ttk.Progressbar(act, mode="indeterminate")
        self.bt_prog.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=10)

        self._bt_info = ttk.Label(tab, text="Prêt")
        self._bt_info.pack(fill=tk.X, padx=10, pady=6)
    ...
    def _tab_performance(self):
        ...
        act = ttk.Frame(tab)
        act.pack(fill=tk.X, padx=10, pady=10)
        ttk.Button(act, text="Afficher Equity", command=self._show_equity).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(act, text="Afficher Drawdown", command=self._show_drawdown).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(act, text="Afficher Trades/Metrics", command=self._show_tables).pack(
            side=tk.LEFT, padx=4
        )

        self._perf_box = ScrolledText(
            tab,
            height=18,
            bg=THEME["panel"],
            fg=THEME["text"],
        ...
        self._perf_box.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    ...
    def _tab_downloads(self):
        ...
        ttk.Label(
            tab,
            text="Module Downloads indisponible.\nCréez threadx/ui/downloads.py avec create_downloads_page(...).",
        ).pack(padx=12, pady=12)
    ...
    def _tab_logs(self):
        ...
        for k, c in dict(
            INFO=THEME["text"], WARNING=THEME["warning"], ERROR=THEME["danger"]
        ).items():
    ...
    def _regen_indicators(self):
        if self.current_data is None or self.bank is None:
            messagebox.showwarning(
                "Indicators",
                "Charge d'abord un dataset et assure-toi que la Bank est dispo.",
            )
            return
    ...
    def trigger_backtest(self):
        ...
        self.bt_prog.start(15)
        self._bt_info.config(text="Backtest en cours…")
        self.status.set("Backtest…")
            ...
            try:
                ...
                if self.bank is not None and self.current_data is not None:
                    try:
                        ...
                        self._log(
                            f"bank.ensure échoué (continuation sans): {e}", "WARNING"
                        )
                ...
                if self.engine and self.current_data is not None:
                    ...
                    run_result = type(
                        ...
                        {
                            "equity": pd.Series(
                                [10000, 10100, 10050], index=self.current_data.index[:3]
                            ),
                            "trades": pd.DataFrame(
                                {"entry_time": [], "exit_time": [], "pnl": []}
                            ),
                            "returns": pd.Series(
                                [0.01, -0.005], index=self.current_data.index[:2]
                            ),
                        },
                    )()
                else:
                ...
                def publish():
                    self.last_equity = (
                        getattr(run_result, "equity", None) if run_result else None
                    )
                    self.last_trades = (
                        getattr(run_result, "trades", None) if run_result else None
                    )
                    self.last_metrics = mets
                    trades_count = (
                        len(self.last_trades) if self.last_trades is not None else 0
                    )
    ...
    def _dump_metrics_to_box(self, mets: Dict[str, Any]):
        self._perf_box.config(state=tk.NORMAL)
        self._perf_box.delete("1.0", tk.END)
        ...
        if self.last_metrics:
            for k in (
                ...
                if k in self.last_metrics:
                    msg.append(f"{k}: {self.last_metrics[k]}")
    ...
    def _export_dialog(self):
        ...
        if self.last_trades is not None and len(self.last_trades):
            p = Path(d) / "trades.csv"
            self.last_trades.to_csv(p, index=False)
            paths.append(p)
        if self.last_equity is not None and len(self.last_equity):
            p = Path(d) / "equity.csv"
            self.last_equity.to_csv(p, header=["equity"])
            paths.append(p)
        if self.last_metrics:
            p = Path(d) / "metrics.json"
            pd.Series(self.last_metrics).to_json(p)
            paths.append(p)
        messagebox.showinfo(
            "Export",
    ...
    def _auto_load_btc_first_found(self):
        ...
        prio = lambda f: (
            0 if "1h" in f.name.lower() else 1 if "15m" in f.name.lower() else 2,
            f.name,
        )
