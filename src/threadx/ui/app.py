    ...
    def create_engine():
        class MockEngine:
            def run(self, *args, **kwargs):
                ...
                @dataclass
                class MockResult:
                    equity: pd.Series = field(
                        default_factory=lambda: pd.Series([10000, 11000])
                    )
                    returns: pd.Series = field(
                        default_factory=lambda: pd.Series([0.0, 0.1])
                    )
...
class ThreadXApp(tk.Tk):
    ...
    def __init__(
        ...
        try:
            ...
            self.logger.info(
                "✅ BacktestEngine réel initialisé avec support GPU/multi-GPU"
            )
        except Exception as e:
    ...
    def _scan_data_directories(self):
        ...
        try:
            # Scan JSON files
            json_files = scan_dir_by_ext(
                str(self.json_dir), {".json", ".ndjson", ".csv"}
            )
            ...
            self.logger.info(
                f"Scanned data: {len(self.available_files)} files, {len(self.by_symbol)} symbols"
            )
    ...
    def _create_home_tab(self):
        ...
        title_label = ttk.Label(
            title_frame, text="ThreadX TechinTerror", font=("Arial", 16, "bold")
        )
        title_label.pack(side=tk.LEFT)

        subtitle_label = ttk.Label(
            title_frame,
            text="Crypto Backtesting Studio - BTC Dashboard",
            font=("Arial", 10),
        )
        ...
        ttk.Label(control_frame, text="Symbol:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 5)
        )
        self.home_symbol_var = tk.StringVar(value="BTCUSDC")
        symbol_combo = ttk.Combobox(
            control_frame,
            textvariable=self.home_symbol_var,
            values=[],
            width=12,
            state="readonly",
        )
        ...
        ttk.Label(control_frame, text="Timeframe:").grid(
            row=0, column=2, sticky=tk.W, padx=(0, 5)
        )
        self.home_timeframe_var = tk.StringVar(value="1h")
        tf_combo = ttk.Combobox(
            control_frame,
            textvariable=self.home_timeframe_var,
        ...
        # Load button
        load_btn = ttk.Button(
            control_frame, text="Load Data", command=self._on_home_load_data
        )
        load_btn.grid(row=0, column=4, padx=(0, 20))

        # Auto-scan button
        scan_btn = ttk.Button(
            control_frame, text="Rescan Files", command=self._on_rescan_files
        )
        ...
        self.stats_text = scrolledtext.ScrolledText(
            stats_frame, height=8, width=30, state=tk.DISABLED, wrap=tk.WORD
        )
        ...
        self.files_text = scrolledtext.ScrolledText(
            files_frame, height=15, width=30, state=tk.DISABLED, wrap=tk.WORD
        )
        ...
        self.chart_canvas.create_text(
            400,
            200,
            text="Chart will appear here after loading data",
    ...
    def _create_data_tab(self):
        ...
        ttk.Label(source_frame, text="Symbol:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        self.symbol_var = tk.StringVar(value="BTCUSDC")
        ttk.Entry(source_frame, textvariable=self.symbol_var, width=15).grid(
            row=0, column=1, sticky=tk.W
        )

        ttk.Label(source_frame, text="Timeframe:").grid(
        ...
        self.data_info_text = scrolledtext.ScrolledText(
            info_frame, height=10, state=tk.DISABLED
        )
    ...
    def _create_indicators_tab(self):
        ...
        config_frame = ttk.LabelFrame(
            main_frame, text="Indicator Configuration", padding="10"
        )
        config_frame.pack(fill=tk.X, pady=(0, 10))

        # Bollinger Bands
        ttk.Label(config_frame, text="BB Period:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        self.bb_period_var = tk.StringVar(value="20")
        ttk.Entry(config_frame, textvariable=self.bb_period_var, width=10).grid(
            row=0, column=1, sticky=tk.W
        )

        ttk.Label(config_frame, text="BB Std:").grid(
            row=0, column=2, sticky=tk.W, padx=(20, 10)
        )
        self.bb_std_var = tk.StringVar(value="2.0")
        ttk.Entry(config_frame, textvariable=self.bb_std_var, width=10).grid(
            row=0, column=3, sticky=tk.W
        )

        # ATR
        ttk.Label(config_frame, text="ATR Period:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 0)
        )
        self.atr_period_var = tk.StringVar(value="14")
        ttk.Entry(config_frame, textvariable=self.atr_period_var, width=10).grid(
            row=1, column=1, sticky=tk.W, pady=(10, 0)
        )

        # Regenerate button
        self.regenerate_btn = ttk.Button(
            config_frame, text="Regenerate Indicators", command=self.trigger_regenerate
        )
        self.regenerate_btn.grid(
            row=2, column=0, columnspan=2, pady=(20, 0), sticky=tk.W
        )

        # Progress
        self.indicators_progress = ttk.Progressbar(config_frame, mode="indeterminate")
        self.indicators_progress.grid(
            row=2, column=2, columnspan=2, pady=(20, 0), sticky=tk.EW, padx=(20, 0)
        )
    ...
    def _create_optimization_tab(self):
        ...
        try:
            ...
            self.logger.info(
                "✅ Onglet d'optimisation paramétrique créé avec moteur unifié"
            )
        except Exception as e:
            # Fallback simple interface
            ...
            ttk.Button(
                controls_frame,
                text="🔄 Réessayer",
                command=self._retry_optimization_init,
            ).pack(side=tk.LEFT, padx=5)
            ttk.Button(
                controls_frame,
                text="📚 Documentation",
                command=self._open_optimization_docs,
            ).pack(side=tk.LEFT, padx=5)
    ...
    def _retry_optimization_init(self):
        ...
        try:
            ...
            if tab_index is not None:
                ...
                messagebox.showinfo(
                    "Succès", "Module d'optimisation rechargé avec succès!"
                )
    ...
    def _create_strategy_tab(self):
        ...
        load_frame = ttk.LabelFrame(
            main_frame, text="Parameter Management", padding="10"
        )
        load_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Button(
            load_frame,
            text="Load Parameters from JSON",
            command=self._load_params_dialog,
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            load_frame, text="Save Parameters to JSON", command=self._save_params_dialog
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            load_frame, text="Reset to Defaults", command=self._reset_params
        ).pack(side=tk.LEFT)

        # Strategy parameters
        params_frame = ttk.LabelFrame(
            main_frame, text="Strategy Parameters", padding="10"
        )
        params_frame.pack(fill=tk.X, pady=(0, 10))

        # Entry parameters
        ttk.Label(params_frame, text="Entry Z-Score:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 10)
        )
        self.entry_z_var = tk.StringVar(value="2.0")
        ttk.Entry(params_frame, textvariable=self.entry_z_var, width=10).grid(
            row=0, column=1, sticky=tk.W
        )

        ttk.Label(params_frame, text="Stop Loss K:").grid(
            row=0, column=2, sticky=tk.W, padx=(20, 10)
        )
        self.k_sl_var = tk.StringVar(value="1.5")
        ttk.Entry(params_frame, textvariable=self.k_sl_var, width=10).grid(
            row=0, column=3, sticky=tk.W
        )

        # Risk parameters
        ttk.Label(params_frame, text="Leverage:").grid(
            row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 0)
        )
        self.leverage_var = tk.StringVar(value="3")
        ttk.Entry(params_frame, textvariable=self.leverage_var, width=10).grid(
            row=1, column=1, sticky=tk.W, pady=(10, 0)
        )

        ttk.Label(params_frame, text="Risk %:").grid(
            row=1, column=2, sticky=tk.W, padx=(20, 10), pady=(10, 0)
        )
        self.risk_var = tk.StringVar(value="2.0")
        ttk.Entry(params_frame, textvariable=self.risk_var, width=10).grid(
            row=1, column=3, sticky=tk.W, pady=(10, 0)
        )

        # Trail parameters
        ttk.Label(params_frame, text="Trail K:").grid(
            row=2, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 0)
        )
        self.trail_k_var = tk.StringVar(value="1.0")
        ttk.Entry(params_frame, textvariable=self.trail_k_var, width=10).grid(
            row=2, column=1, sticky=tk.W, pady=(10, 0)
        )

        # Validation button
        ttk.Button(
            params_frame, text="Validate Parameters", command=self._validate_params
        ).grid(row=3, column=0, columnspan=2, pady=(20, 0), sticky=tk.W)
        ...
        self.params_text = scrolledtext.ScrolledText(
            current_frame, height=10, state=tk.DISABLED
        )
    ...
    def _create_backtest_tab(self):
        ...
        ttk.Button(exec_frame, text="Cancel", command=self._cancel_backtest).pack(
            side=tk.LEFT
        )
        ...
        ttk.Checkbutton(
            options_frame, text="Use GPU Acceleration", variable=self.use_gpu_var
        ).pack(side=tk.LEFT, padx=(0, 20))

        self.cache_indicators_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            options_frame, text="Cache Indicators", variable=self.cache_indicators_var
        ).pack(side=tk.LEFT)

        # Results summary
        results_frame = ttk.LabelFrame(
            main_frame, text="Backtest Results", padding="10"
        )
        results_frame.pack(fill=tk.BOTH, expand=True)

        self.results_text = scrolledtext.ScrolledText(
            results_frame, height=15, state=tk.DISABLED
        )
    ...
    def _create_performance_tab(self):
        ...
        ttk.Button(
            charts_frame, text="Show Equity Curve", command=self._show_equity_chart
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            charts_frame, text="Show Drawdown", command=self._show_drawdown_chart
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            charts_frame, text="Export Charts", command=self._export_charts
        ).pack(side=tk.LEFT)
        ...
        ttk.Button(
            tables_frame, text="Show Trades", command=self._show_trades_table
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(
            tables_frame, text="Show Metrics", command=self._show_metrics_table
        ).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(tables_frame, text="Export Data", command=self._export_data).pack(
            side=tk.LEFT
        )

        # Performance display
        perf_frame = ttk.LabelFrame(
            main_frame, text="Performance Summary", padding="10"
        )
        perf_frame.pack(fill=tk.BOTH, expand=True)

        self.performance_text = scrolledtext.ScrolledText(
            perf_frame, height=12, state=tk.DISABLED
        )
    ...
    def _create_logs_tab(self):
        ...
        ttk.Button(controls_frame, text="Clear Logs", command=self._clear_logs).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(controls_frame, text="Export Logs", command=self._export_logs).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ...
        self.logs_text = scrolledtext.ScrolledText(
            main_frame, height=25, state=tk.DISABLED
        )
    ...
    def trigger_regenerate(self) -> None:
        ...
        if self._is_task_running("regenerate"):
            messagebox.showwarning(
                "Warning", "Indicator regeneration already in progress"
            )
            return
        ...
        def regenerate_task():
            ...
            try:
                ...
                self.logger.info(
                    f"Starting indicator regeneration for {symbol} {timeframe}"
                )
                ...
                if self.current_df is not None and not self.current_df.empty:
                    ...
                    self.logger.warning(
                        "Aucune donnée disponible pour calculer les indicateurs"
                    )
                    result = False
    ...
    def trigger_backtest(self) -> None:
        ...
        def backtest_task():
            ...
            try:
                ...
                self.logger.info(
                    f"🎯 Démarrage backtest ThreadX: {params['symbol']} {params['timeframe']}"
                )
                start_time = time.time()
                ...
                if not hasattr(self, "current_data") or self.current_data.empty:
                    raise ValueError(
                        "Aucune donnée chargée. Veuillez charger des données d'abord."
                    )
                ...
                indicators = {"bollinger": bollinger_result, "atr": atr_result}
                ...
                metrics = self.performance.summarize(
                    trades=result.trades, returns=result.returns
                )
                elapsed = time.time() - start_time
                ...
                self.logger.info(
                    f"   Return total: {((result.equity.iloc[-1] / result.equity.iloc[0]) - 1) * 100:.2f}%"
                )
...
"""
    ...
    def _has_results(self) -> bool:
        ...
        return (
            self.last_returns is not None
            and self.last_trades is not None
        ...
        if file_path:
            try:
                ...
                with open(file_path, "w") as f:
                    json.dump(params, f, indent=2)
                messagebox.showinfo(
                    "Success", f"Parameters saved to {Path(file_path).name}"
                )
            except Exception as e:
        ...
        if file_path:
            try:
                ...
                with open(file_path, "w") as f:
                    f.write(content)
                messagebox.showinfo(
                    "Success", f"Logs exported to {Path(file_path).name}"
                )
            except Exception as e:
    ...
    def _show_equity_chart(self):
        ...
        try:
            ...
            equity = (
                self.last_equity
                if hasattr(self, "last_equity") and self.last_equity is not None
                else (1 + self.last_returns).cumprod() * 10000
            )
    ...
    def _show_drawdown_chart(self):
        ...
        try:
            ...
            if chart_path:
                messagebox.showinfo(
                    "Chart", f"Drawdown chart saved to {chart_path.name}"
                )
    ...
    def _export_charts(self):
        ...
        try:
            ...
            equity_path = plot_equity(
                equity, save_path=Path(dir_path) / "equity_curve.png"
            )
            if equity_path:
                exported_files.append(equity_path)

            drawdown_path = plot_drawdown(
                equity, save_path=Path(dir_path) / "drawdown.png"
            )
            if drawdown_path:
            ...
            messagebox.showinfo(
                "Export",
                f"Exported {len(exported_files)} charts to {Path(dir_path).name}",
            )
    ...
    def _export_data(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Export",
                f"Exported {len(exported_files)} files to {Path(dir_path).name}",
            )
    ...
    def _on_home_load_data(self):
        ...
        try:
            ...
            if not symbol or not timeframe:
                messagebox.showwarning(
                    "Missing Selection", "Please select both symbol and timeframe"
                )
                return
    ...
    def _on_start_download(self):
        ...
        try:
            ...
            if not tokens_str or not start_date or not end_date:
                messagebox.showwarning(
                    "Missing Parameters", "Please fill in all required fields"
                )
                return
            ...
            try:
                ...
                messagebox.showerror(
                    "Invalid Dates", "Please use YYYY-MM-DD format for dates"
                )
                return
                    ...
                    for i, token in enumerate(tokens):
                        ...
                        if not dry_run:
                            ...
                            try:
                                ...
                                results.append(
                                    f"✓ {token} 1m: {len(df_1m)} bars downloaded"
                                )
                                ...
                                if verify_3h:
                                    ...
                                    results.append(
                                        f"✓ {token} 3h: {len(df_3h)} bars resampled"
                                    )
                                    ...
                                    if len(df_3h) > 0:
                                        ...
                                        results.append(
                                            f"  → 3h verification: FAILED (empty)"
                                        )
                            ...
                            results.append(
                                f"✓ {token}: Would download 1m data for {start_date} to {end_date}"
                            )
                            if verify_3h:
                                results.append(
                                    f"✓ {token}: Would verify with 3h resampling"
                                )
                        ...
                        self.after(
                            ...
                            lambda text="\n".join(
                                results
                            ): self._update_download_results(text),
                        )
...
def run_app() -> None:
    ...
    try:
        ...
        error_msg = (
            f"Failed to start ThreadX Application: {e}\n{traceback.format_exc()}"
        )
