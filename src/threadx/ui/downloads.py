...
class DownloadsPage(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            title_frame, text="📥 Téléchargements OHLCV", font=("Arial", 14, "bold")
        )
    ...
    def create_config_section(self):
        ...
        self.symbols_listbox = tk.Listbox(
            listbox_frame, selectmode="extended", height=4
        )
        scrollbar = ttk.Scrollbar(
            listbox_frame, orient="vertical", command=self.symbols_listbox.yview
        )
    ...
    def create_advanced_options(self, parent):
        ...
        ttk.Checkbutton(
            options_row1,
            text="✅ Vérification 3h (sanity check alignment)",
        ...
        ttk.Entry(tf_frame, textvariable=self.timeframes_var, width=50).pack(
            side="left", padx=(10, 0)
        )
    ...
    def create_controls_section(self):
        ...
        self.download_btn = ttk.Button(
            control_frame, text="📥 Télécharger", command=self.start_download
        )
        ...
        self.clear_logs_btn = ttk.Button(
            control_frame, text="🗑️ Vider logs", command=self.clear_logs
        )
    ...
    def start_download(self):
        ...
        logger.info(
            f"🚀 Démarrage téléchargement: {len(symbols)} symboles, {start_date.date()} → {end_date.date()}"
        )
    ...
    def download_worker(self, params: Dict[str, Any]):
        ...
        try:
                ...
                try:
                    if dry_run:
                        ...
                        if self.ingestion_manager:
                            self.real_download(
                                symbol, start_date, end_date, timeframes, params
                            )
                        else:
                            self.log_queue.put(
                                f"⚠️ IngestionManager non disponible pour {symbol}"
                            )
                    ...
                    if params["enable_verification"] and not self.should_stop:
                        ...
                        self.progress_queue.put(
                            ("progress", progress, f"Vérification {symbol} terminée")
                        )
            ...
            if not self.should_stop:
                self.log_queue.put("─" * 50)
                self.log_queue.put(
                    f"✅ Téléchargement terminé: {completed_tasks}/{total_tasks} tâches"
                )
