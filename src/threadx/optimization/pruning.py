...
def pareto_soft_prune(
    ...
    logger.info(f"Pruning Pareto soft: {len(df)} configurations, "
    ...
    for metric in metrics:
        ...
        logger.debug(f"Métrique {metric}: seuil {quantile:.2%} = {threshold:.4f} "
    ...
    for i, row in df_work.iterrows():
        ...
        if is_pareto_candidate:
            ...
            if stagnation_counter >= patience:
                logger.info(f"Early stopping activé à l'itération {i} "
    ...
    if pareto_front:
        ...
        df_kept = df_work.nlargest(1, primary_metric) if is_maximize else df_work.nsmallest(1, primary_metric)
    ...
    logger.info(f"Pruning terminé: {len(df_kept)}/{n_original} configurations conservées "
...
def _is_maximize_metric(metric: str) -> bool:
    ...
    maximize_metrics = {
        'pnl', 'profit', 'return', 'returns', 'sharpe', 'sharpe_ratio',
        'sortino', 'sortino_ratio', 'calmar', 'win_rate', 'success_rate'
    }
    
    minimize_metrics = {
        'drawdown', 'max_drawdown', 'max_dd', 'volatility', 'var', 'cvar',
        'loss', 'risk', 'downside_deviation'
    }
...
def _dominates(solution_a: pd.Series, solution_b: pd.Series, metrics: Tuple[str, ...]) -> bool:
    """
...
if __name__ == "__main__":
    ...
    print(f"Métadonnées: {metadata['pruning_rate']:.1%} pruned, "
