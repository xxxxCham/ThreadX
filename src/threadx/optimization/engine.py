...
class SweepRunner:
    ...
    def run_grid(self, grid_spec: ScenarioSpec, *, reuse_cache: bool = True) -> pd.DataFrame:
        """
    ...
    def run_monte_carlo(self, mc_spec: ScenarioSpec, *, reuse_cache: bool = True) -> pd.DataFrame:
        """
        ...
        results_df = self._execute_combinations_with_pruning(scenarios, reuse_cache=reuse_cache)
        
        return results_df
    
    def _execute_combinations(self, combinations: List[Dict], *, reuse_cache: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            with self._time_stage("strategy_evaluation"):
                for i, combo in enumerate(combinations):
                    ...
                    result = self._evaluate_single_combination(combo, computed_indicators)
    ...
    def _execute_combinations_with_pruning(self, combinations: List[Dict], *, reuse_cache: bool = True) -> pd.DataFrame:
                ...
                for batch_start in range(0, len(combinations), batch_size):
                    ...
                    if len(results) >= 200:  # Seuil minimum pour pruning
                        ...
                        if pruning_metadata['early_stop_triggered']:
                            self.logger.info("Early stopping déclenché par pruning Pareto")
                            break
    ...
    def _extract_unique_indicators(self, combinations: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    def _compute_batch_indicators(self, unique_indicators: Dict[str, List[Dict]], *, reuse_cache: bool = True) -> Dict[str, Dict]:
    ...
    def _evaluate_single_combination(self, combo: Dict, computed_indicators: Dict) -> Dict:
        ...
        for param_name, param_value in combo.items():
            if param_name.startswith('bb_'):
                ...
                bb_params = {name[3:]: value for name, value in combo.items() if name.startswith('bb_')}
                params_key = self._params_to_key(bb_params)
                combo_indicators['bollinger'] = computed_indicators.get(indicator_type, {}).get(params_key)
            
            elif param_name.startswith('atr_'):
                indicator_type = 'atr'
                atr_params = {name[4:]: value for name, value in combo.items() if name.startswith('atr_')}
                params_key = self._params_to_key(atr_params)
                combo_indicators['atr'] = computed_indicators.get(indicator_type, {}).get(params_key)
    ...
    def _generate_strategy_signals(self, indicators: Dict, params: Dict) -> np.ndarray:
        ...
        if 'bollinger' in indicators and 'atr' in indicators:
            ...
            bb_signals = (noise > bb_std * 0.5).astype(int) - (noise < -bb_std * 0.5).astype(int)
    ...
    def _log_progress(self):
        ...
        if self.total_scenarios > 0:
            ...
            self.logger.info(f"Progrès: {self.current_scenario}/{self.total_scenarios} "
    ...
    def _log_final_stats(self, pruning_metadata: Optional[Dict] = None):
        ...
        self.logger.info(f"Sweep terminé: {self.current_scenario} scénarios "
        ...
        for stage, timings in self.stage_timings.items():
            ...
            self.logger.info(f"  {stage}: {total_stage_time:.2f}s "
                           f"(avg: {avg_time:.3f}s, {len(timings)} appels)")
        
        # Métadonnées de pruning
        if pruning_metadata:
            self.logger.info(f"Pruning Pareto: {pruning_metadata['pruned_count']} configurations éliminées "
