...
def summarize_distribution(results_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in numeric_cols:
        ...
        stats = {
            ...
            'success_rate': float((series > 0).mean()) if 'pnl' in col.lower() or 'return' in col.lower() else None,
            'negative_rate': float((series < 0).mean()) if 'pnl' in col.lower() or 'return' in col.lower() else None
        }
    ...
    if summary:
        summary['_meta'] = {
            ...
            'missing_data_pct': float(results_df.isnull().sum().sum() / (len(results_df) * len(results_df.columns)))
        }
...
def build_heatmaps(results_df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    if len(param_cols) < 2:
        logger.warning(f"Nombre insuffisant de paramètres pour heatmaps: {len(param_cols)}")
        return {}
...
def _identify_parameter_columns(df: pd.DataFrame) -> List[str]:
    ...
    param_indicators = [
        'period', 'std', 'window', 'length', 'threshold', 'alpha', 'beta',
        'gamma', 'lambda', 'factor', 'multiplier', 'ratio', 'size', 'step'
    ]
    ...
    for col in df.columns:
        ...
        if df[col].dtype in [np.int64, np.float64, int, float]:
            ...
            elif any(col_lower.startswith(prefix) for prefix in ['bb_', 'atr_', 'rsi_', 'ma_', 'ema_']):
                param_cols.append(col)
...
def _identify_metric_columns(df: pd.DataFrame) -> List[str]:
    ...
    metric_indicators = [
        'pnl', 'profit', 'return', 'sharpe', 'sortino', 'calmar',
        'drawdown', 'volatility', 'var', 'win_rate', 'success_rate'
    ]
...
def _build_metric_heatmaps(df: pd.DataFrame, param_cols: List[str], metric: str) -> Dict[str, pd.DataFrame]:
    ...
    *,
    ...
    manifest = {
        ...
        'data_summary': {
            ...
            'numeric_columns': list(results_df.select_dtypes(include=[np.number]).columns),
...
def _quick_performance_summary(results_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for metric in key_metrics:
        matching_cols = [col for col in results_df.columns if metric.lower() in col.lower()]
        ...
        if matching_cols:
            ...
            if len(series) > 0:
                summary[metric] = {
                    'best': float(series.max() if metric not in ['max_drawdown'] else series.min()),
                    'worst': float(series.min() if metric not in ['max_drawdown'] else series.max()),
                    'mean': float(series.mean()),
                    'median': float(series.median())
                }
...
def validate_results_dataframe(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    if missing_pct > 0.5:
        validation['warnings'].append(f"Taux élevé de valeurs manquantes: {missing_pct:.1%}")
...
if __name__ == "__main__":
    ...
    print(f"Test reporting: {len(stats)} métriques analysées, "
