...
def generate_param_grid(spec: ScenarioSpec) -> List[Dict[str, Any]]:
    ...
    logger.info(
        f"Après dé-duplication: {len(unique_combinations)} combinaisons finales"
    )
...
def generate_monte_carlo(spec: ScenarioSpec) -> List[Dict[str, Any]]:
    ...
    """
    logger.info(
        f"Génération Monte Carlo: {spec['n_scenarios']} scénarios, "
    ...
    for i, sample in enumerate(samples):
        ...
        # Application des contraintes
        if not spec.get("constraints") or _evaluate_constraints(
            scenario, spec["constraints"]
        ):
            scenarios.append(scenario)

    logger.info(
        f"Monte Carlo généré: {len(scenarios)} scénarios valides "
...
def _transform_sample_value(
    sample: float, param_spec: Dict[str, Any]
) -> Union[int, float]:
...
def validate_scenario_spec(spec: Dict[str, Any]) -> ScenarioSpec:
    ...
    validated = {
        ...
        "sampler": spec.get(
            "sampler", "sobol" if spec.get("type") == "monte_carlo" else "grid"
        ),
...
if __name__ == "__main__":
    ...
    test_spec = ScenarioSpec(
        type="grid",
        params={"bb_period": [10, 20, 30], "bb_std": [1.5, 2.0, 2.5]},
        seed=42,
        n_scenarios=100,
        sampler="grid",
        constraints=[],
