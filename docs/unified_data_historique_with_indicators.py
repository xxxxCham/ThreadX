...
# =========================================================
#  Chemins TradXPro (structure normalisée)
# =========================================================
...
OUTPUT_DIR = os.getenv("TRADX_OUTPUT_DIR", r"D:\TradXPro\best_token_DataFrame")

# Chemins historiques pour compatibilité
DEFAULT_JSON_PATH = r"D:\\TradXPro\\scripts\\mise_a_jour_dataframe\\resultats_choix_des_100tokens.json" if IS_WINDOWS else "/home/user/resultats_choix_des_100tokens.json"
DEFAULT_LOG_FILE = r"D:\\TradXPro\\scripts\\mise_a_jour_dataframe\\unified_data_historique.log" if IS_WINDOWS else "/home/user/unified_data_historique.log"
...
def _fix_dataframe_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    timestamp_cols = [col for col in df.columns if col.lower() in ["timestamp", "datetime", "time", "open_time"]]
...
def indicator_path(symbol: str, tf: str, name: str, key: str) -> str:
    """Chemin indicateur selon conventions TradXPro.
    
    Args:
        symbol: Symbole crypto
        tf: Timeframe
        name: Nom indicateur
        key: Clé paramètres (ex: 'p14', 'p20_s2.2', 'f12_s26_sig9')
    """
    return os.path.join(INDICATORS_DB_ROOT, symbol.upper(), tf.lower(), f"{name}_{key}.parquet")
...
def get_all_available_symbols() -> List[str]:
    ...
    base_symbols = ["BTCUSDC", "ETHUSDC", "ADAUSDC", "SOLUSDC", "DOTUSDC",
                   "AVAXUSDC", "MATICUSDC", "LINKUSDC", "UNIUSDC", "ATOMUSDC"]
...
# =========================================================
#  NOUVEAU: Cache et déduplication logging
# =========================================================
_LOGGED_MESSAGES = set()
_OHLCV_CACHE = {}  # Cache LRU simple

def log_once(message: str, level: str = "INFO", force: bool = False):
    """Log un message une seule fois (anti-doublon)."""
    global _LOGGED_MESSAGES
    
    if not force and message in _LOGGED_MESSAGES:
        return
...
# =========================================================
#  FIX CRITIQUE: Conversion timestamps avec ms + UTC
# =========================================================

def _fix_timestamp_conversion(df: pd.DataFrame, timestamp_col: str = 'timestamp') -> pd.DataFrame:
    """Corrige la conversion timestamp avec gestion millisecondes et UTC forcé."""
    if timestamp_col not in df.columns:
        # Chercher automatiquement la colonne timestamp
        timestamp_cols = [col for col in df.columns if col.lower() in ['timestamp', 'open_time', 'close_time', 'datetime']]
        if not timestamp_cols:
            raise ValueError("Aucune colonne timestamp trouvée")
        timestamp_col = timestamp_cols[0]
    
    # Conversion avec détection automatique ms/s + UTC forcé
    timestamp_series = pd.to_numeric(df[timestamp_col], errors='coerce')
    
    # Si valeur > 1e12, c'est des millisecondes, sinon secondes
    if timestamp_series.max() > 1e12:
        df['datetime'] = pd.to_datetime(timestamp_series, unit='ms', utc=True)
    else:
        df['datetime'] = pd.to_datetime(timestamp_series, unit='s', utc=True)
    
    # Nettoyage et tri OBLIGATOIRE
    df = df.dropna(subset=['datetime'])
    df = df.sort_values('datetime')
    df = df.drop_duplicates('datetime', keep='last')
    df = df.set_index('datetime')
    
    # Supprimer l'ancienne colonne timestamp
    if timestamp_col in df.columns:
        df = df.drop(columns=[timestamp_col])
    
    # Validation finale
    assert df.index.is_monotonic_increasing, "Index temporel non ordonné"
    assert df.index.tz is not None, "Index sans timezone UTC"
    
    logger.debug(f"Timestamps corrigés: {len(df)} lignes, {df.index[0]} → {df.index[-1]}")
    return df

def _sanitize_ohlcv_df(df: pd.DataFrame) -> pd.DataFrame:
    """Nettoie et valide un DataFrame OHLCV."""
    ...
    # Nettoyage NaN et validation
    initial_len = len(df)
    df = df.dropna(subset=required_cols)
    if len(df) < initial_len:
        logger.warning(f"Supprimé {initial_len - len(df)} lignes avec NaN")
    
    # Types optimisés
    df = df.astype({col: 'float32' for col in required_cols})
    
    # Validation finale
    assert df['close'].notna().all(), "Prix de clôture contient des NaN"
    assert (df['high'] >= df['low']).all(), "High < Low détecté"
    assert (df['volume'] >= 0).all(), "Volume négatif détecté"
    
    return df

# =========================================================
#  FIX CRITIQUE: Calcul périodes sûres réalistes
# =========================================================

def _calculate_safe_periods(df: pd.DataFrame, timeframe: str) -> dict:
    """Calcule les périodes sûres basées sur la longueur réelle des données."""
    if len(df) == 0:
        return {"max_safe": 0, "warmup": 0, "available_days": 0}
    
    # Périodes de warmup par indicateur (valeurs RÉALISTES)
    warmup_periods = {
        'rsi': 14,
        'bollinger': 20, 
        'ema': 20,
    ...
    max_warmup = max(warmup_periods.values())  # 35
    
    # Calcul période disponible RÉELLE
    start_dt = pd.to_datetime(df.index[0])
    end_dt = pd.to_datetime(df.index[-1])
    available_days = (end_dt - start_dt).days
    
    # Période sûre = données disponibles - warmup
    max_safe_period = max(0, len(df) - max_warmup)
    
    # Seuils d'avertissement par timeframe
    min_recommended = {
        '1h': 96,    # 4 jours
        '30m': 192,  # 4 jours  
        '15m': 384,  # 4 jours
    ...
    is_short_data = max_safe_period < min_recommended.get(timeframe, 100)
    
    return {
        "max_safe": max_safe_period,
        "warmup": max_warmup,
        "available_days": available_days,
        "total_candles": len(df),
        "is_short": is_short_data,
        "warmup_periods": warmup_periods
    }

    """Formatage prix adaptatif selon l'échelle pour éviter 0.00."""
    if pd.isna(price) or price == 0:
        return "0.00"
    
    # Utiliser le prix lui-même si pas de médiane
    reference = median_price if median_price is not None else price
    
    if reference >= 1:
        return f"{price:.2f}"
    elif reference >= 0.1:
        return f"{price:.4f}"
    elif reference >= 0.001:
        return f"{price:.6f}"
    else:
        return f"{price:.8f}"

# =========================================================
#  FIX CRITIQUE: Gestion symboles indisponibles
# =========================================================

@lru_cache(maxsize=1)
def get_binance_available_symbols() -> set:
    """Cache des symboles disponibles chez Binance."""
    try:
        url = "https://api.binance.com/api/v3/exchangeInfo"
        resp = requests.get(url, timeout=10).json()
        symbols = {s["symbol"] for s in resp.get("symbols", []) if s["status"] == "TRADING"}
        log_once(f"Binance: {len(symbols)} symboles actifs récupérés")
        return symbols
    except Exception as e:
        logger.error(f"Erreur récupération symboles Binance: {e}")
        return set()

def filter_available_symbols(requested_symbols: List[str]) -> Tuple[List[str], List[str]]:
    """Filtre les symboles disponibles chez Binance."""
    available_symbols = get_binance_available_symbols()
    ...
    if invalid:
        logger.warning(f"Symboles non disponibles chez Binance: {invalid}")
...
# =========================================================
#  FIX CRITIQUE: Formatage prix adaptatif
# =========================================================

def _format_price_adaptive(price: float, median_price: float = None) -> str:
    """Formatage prix adaptatif selon l'échelle pour éviter 0.00."""
    if pd.isna(price) or price == 0:
        return "0.00"
